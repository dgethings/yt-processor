...
async function saveMarkdownFile(content, filename, directory) {
    try {
...
export default tool({
    ...
    async execute(args, context) {
        try {
            ...
            if (DEBUG)
                console.log(`[youtube-processor] Fetching metadata and transcript for ${videoId}`);
            ...
            if (videoInfo.transcript && !videoInfo.transcript.includes("No transcript available")) {
                if (DEBUG)
                    console.log(`[youtube-processor] Generating summary for transcript (${videoInfo.transcript.length} chars)`);
